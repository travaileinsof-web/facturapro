...
try {
    ...
    $queries = [
        ...
        "CREATE TABLE IF NOT EXISTS ProformaInvoice (
            id VARCHAR(50) PRIMARY KEY, accountId VARCHAR(50) NOT NULL, number VARCHAR(100) NOT NULL, clientId VARCHAR(50) NOT NULL, items TEXT NOT NULL, subtotal DECIMAL(15, 2) DEFAULT 0, taxRate DECIMAL(5, 2) DEFAULT 0, taxAmount DECIMAL(15, 2) DEFAULT 0, discount DECIMAL(15, 2) DEFAULT 0, total DECIMAL(15, 2) DEFAULT 0, status VARCHAR(50) DEFAULT 'brouillon', type VARCHAR(50) DEFAULT 'facture', issueDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP, dueDate TIMESTAMP NULL, lastReminderDate TIMESTAMP NULL, notes TEXT NULL, createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (accountId) REFERENCES Account(id) ON DELETE CASCADE, FOREIGN KEY (clientId) REFERENCES Client(id) ON DELETE RESTRICT
        )",
        "CREATE TABLE IF NOT EXISTS Receipt (
            id VARCHAR(50) PRIMARY KEY, accountId VARCHAR(50) NOT NULL, number VARCHAR(100) NOT NULL, proformaInvoiceId VARCHAR(50) NULL, clientId VARCHAR(50) NOT NULL, amount DECIMAL(15, 2) NOT NULL, paymentMethod VARCHAR(50) NULL, paymentDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP, notes TEXT NULL, createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (accountId) REFERENCES Account(id) ON DELETE CASCADE, FOREIGN KEY (proformaInvoiceId) REFERENCES ProformaInvoice(id) ON DELETE RESTRICT, FOREIGN KEY (clientId) REFERENCES Client(id) ON DELETE RESTRICT
        )",
